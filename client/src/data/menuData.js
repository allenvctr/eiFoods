export const MENU_DO_DIA = [
  {
    id: 1,
    nome: 'Arroz + Frango Assado + Batata',
    preco: 250,
    imagem: 'https://images.unsplash.com/photo-1598103442097-8b74394b95c4?auto=format&fit=crop&w=400&q=80',
    descricao: 'Frango assado no forno com arroz branco e batata frita crocante',
  },
  {
    id: 2,
    nome: 'Peixe Grelhado + Arroz',
    preco: 220,
    imagem: 'https://images.unsplash.com/photo-1467003909585-2f8a72700288?auto=format&fit=crop&w=400&q=80',
    descricao: 'Peixe fresco grelhado com arroz e salada de tomate',
  },
  {
    id: 3,
    nome: 'Arroz + Carne Guisada + Salada',
    preco: 270,
    imagem: 'https://images.unsplash.com/photo-1574484284002-952d92456975?auto=format&fit=crop&w=400&q=80',
    descricao: 'Carne guisada no ponto com arroz e salada fresca',
  },
  {
    id: 4,
    nome: 'Frango Grelhado + Massa + Legumes',
    preco: 260,
    imagem: 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?auto=format&fit=crop&w=400&q=80',
    descricao: 'Frango grelhado com massa e mix de legumes salteados',
  },
  {
    id: 5,
    nome: 'Feijão Guisado + Arroz + Ovo',
    preco: 180,
    imagem: 'https://images.unsplash.com/photo-1512058564366-18510be2db19?auto=format&fit=crop&w=400&q=80',
    descricao: 'Feijão guisado com arroz e ovo estrelado',
  },
  {
    id: 6,
    nome: 'Caril de Frango + Arroz Basmati',
    preco: 290,
    imagem: 'https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?auto=format&fit=crop&w=400&q=80',
    descricao: 'Caril aromático com frango e arroz basmati perfumado',
  },
]

export const OPCOES_MOLHO = ['Sem Molho', 'Com Molho']
export const OPCOES_PIRIPIRI = ['Sem Piripiri', 'Com Piripiri']
export const OPCOES_SAL = ['Sem Sal', 'Pouco Sal', 'Normal']

export const EXTRAS_PAGOS = [
  { id: 'frango', nome: '+ Frango extra', preco: 50 },
  { id: 'ovo', nome: '+ Ovo estrelado', preco: 30 },
  { id: 'queijo', nome: '+ Queijo', preco: 40 },
  { id: 'salada', nome: '+ Salada extra', preco: 25 },
]

export const CONFIG = {
  whatsappNumero: '258841234567',
  horaEntrega: '12h',
  nomeEmpresa: 'Marmita Fresca',
}
